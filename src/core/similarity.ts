...
import { EmbeddingIndex, EmbeddingCache } from "./embedding";
...
export function searchCompartment(
	...
	index: EmbeddingIndex,
	compartment: keyof Pick<EmbeddingCache, "title" | "tags" | "links" | "content">,
	topK: number
): SimilarityResult[] {
	...
	for (const path of candidates) {
		...
		if (!cached) continue;
		const vec = cached[compartment] as number[];
...
export function retrieveSimilar(
	sourceCached: EmbeddingCache,
	candidates: Set<string>,
	index: EmbeddingIndex,
	topK: number
): RetrievalResults {
	return {
		title: searchCompartment(sourceCached.title, candidates, index, "title", topK),
		tags: searchCompartment(sourceCached.tags, candidates, index, "tags", topK),
		links: searchCompartment(sourceCached.links, candidates, index, "links", topK),
		content: searchCompartment(sourceCached.content, candidates, index, "content", topK),
	};
}
