...
describe("searchCompartment", () => {
	function makeShadow(vec: number[]): ShadowFile {
		return {
			mtime: 0,
			title: vec,
			tags: vec,
	...
	it("returns top-K results sorted by score", () => {
		...
		index.set("close.md", makeShadow([1, 0]));
		index.set("far.md", makeShadow([0, 1]));
		index.set("medium.md", makeShadow([0.7, 0.7]));
	...
	it("returns fewer than K if not enough candidates", () => {
		const index = new EmbeddingIndex();
		index.set("only.md", makeShadow([1, 0]));
	...
	it("skips candidates not in index", () => {
		const index = new EmbeddingIndex();
		index.set("exists.md", makeShadow([1, 0]));
