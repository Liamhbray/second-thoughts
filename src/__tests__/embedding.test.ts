...
import { hashPath, EmbeddingIndex, EmbeddingCache } from "../core/embedding";
...
describe("EmbeddingIndex", () => {
	function makeCache(): EmbeddingCache {
		return {
			mtime: Date.now(),
			title: [1, 2, 3],
			tags: [4, 5, 6],
	...
	it("stores and retrieves entries", () => {
		...
		index.set("test.md", cached);
		expect(index.get("test.md")).toBe(cached);
	});
	...
	it("tracks size", () => {
		...
		index.set("a.md", makeCache());
		index.set("b.md", makeCache());
		expect(index.size()).toBe(2);
	});

	it("provides reverse hash lookup", () => {
		const index = new EmbeddingIndex();
		index.set("notes/test.md", makeCache());
